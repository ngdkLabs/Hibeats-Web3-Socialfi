  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (result === 'like_batched') {
        toast.success(wasLiked ? "Unlike queued!" : "Like queued!");
      ...
      if (isQuote) {
        ...
        try {
          ...
          if (result === 'repost_batched') {
            toast.success(wasReposted ? "Unrepost queued!" : "Repost queued!");
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-b py-3">
                    ...
                    <div className="flex items-center space-x-8">
                      ...
                      <Button 
                        ...
                        variant="ghost" 
                        size="sm" 
                        className={`h-8 px-2 space-x-2 ${post.isLiked ? 'text-red-500' : 'text-muted-foreground hover:text-red-500'} hover:bg-red-500/10 transition-all duration-200`}
                        onClick={handleLike}
                        disabled={isLiking}
                      >
                        <Heart className={`w-4 h-4 transition-all duration-200 ${post.isLiked ? 'fill-current' : ''} ${isAnimating ? 'heart-burst' : post.isLiked ? 'heart-pop' : ''}`} />
                        <span className="text-xs transition-all duration-200">{post.likes}</span>
                      </Button>
                    </div>

                    {/* Right side: Bookmark, Share, Tip */}
                    <div className="flex items-center space-x-2">
                      ...
                      {/* Tip Button */}
                      {post && post.author !== smartAccountAddress && (
                        <TipButton
                          targetType={TipTarget.POST}
                          targetId={post.id}
                          recipientAddress={post.author}
                          recipientName={post.authorProfile?.displayName}
                          recipientAvatar={post.authorProfile?.avatarHash ? getIPFSUrl(post.authorProfile.avatarHash) : undefined}
                          targetTitle={post.content.slice(0, 50)}
                          variant="ghost"
                          size="sm"
                          showIcon={true}
                          showText={false}
